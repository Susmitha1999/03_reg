...
The above one is not very informative.So, let try different way.
...
# This plot visually giving evidence that quadratic model maybe better fit. Let's try that.
...
# Model fitting
mod1_int <- lmer(yield_mgha ~ 1 + (1| rep), #Intercept only model with random effects of rep
                 data = reg_dfw) 
...
mod1_int_randeff <- ranef(mod1_int)[[1]] #Random effects 

mod1_int_randeff

ggplot(mod1_int_randeff,
       aes(sample = `(Intercept)` )) + # `` acsent, ''single quotation marks
  stat_qq() + 
  stat_qq_line()
...
nd <- nd %>%
  mutate(mod1_yield_mgha = predict(mod1_int,
                                   nd,
                                   re.form = NA # Do not consider random effects
                                   ))
## As it is intercept only model, yield is same for all seed rates
...
# we don't have enough variance to estimate b and xs random effects.
